...
public class DatasetListttt extends AppCompatActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (toolbar != null) {
            ...
            toolbar.setNavigationOnClickListener(v -> {
                Intent intent = new Intent(DatasetListttt.this, AdminDashboard.class);
        ...
        adapter = new DatasetAdapter(new DatasetAdapter.Listener() {
            ...
            public void onDownload(DatasetItem d) {
                ...
                if (rel == null || rel.trim().isEmpty()) {
                    Toast.makeText(DatasetListttt.this, "Tidak ada file untuk diunduh", Toast.LENGTH_SHORT).show();
                    return;
                }
        ...
        RetrofitClient.api().listDataset().enqueue(new Callback<List<DatasetItem>>() {
            ...
            public void onResponse(Call<List<DatasetItem>> call, retrofit2.Response<List<DatasetItem>> resp) {
                if (resp.isSuccessful() && resp.body() != null) adapter.setData(resp.body());
                    ...
                    Toast.makeText(DatasetListttt.this, "Gagal load dataset " + resp.code(), Toast.LENGTH_SHORT).show();
            }

            @Override
            public void onFailure(Call<List<DatasetItem>> call, Throwable t) {
                Toast.makeText(DatasetListttt.this, "Error: " + t.getMessage(), Toast.LENGTH_LONG).show();
            }
        });
    }
        ...
        RetrofitClient.api().listSampel(null).enqueue(new Callback<List<Sampel>>() {
            ...
            public void onResponse(Call<List<Sampel>> call, retrofit2.Response<List<Sampel>> resp) {
                if (!resp.isSuccessful() || resp.body() == null) {
                    Toast.makeText(DatasetListttt.this, "Gagal ambil sampel (" + resp.code() + ")", Toast.LENGTH_SHORT).show();
                    return;
                }
                ...
                new AlertDialog.Builder(DatasetListttt.this)
    ...
    private void askTrainOptionsThenBuild(List<Sampel> items, Map<String, Integer> count) {
        ...
        new AlertDialog.Builder(this)
                ...
                .setSingleChoiceItems(kChoices, 1, (dialog, which) -> selectedKFold = kValues[which])
        ...
        new Thread(() -> {
            try {
                ...
                main.post(() -> {
                    ...
                    Toast.makeText(DatasetListttt.this, "Gagal: " + e.getMessage(), Toast.LENGTH_LONG).show();
                });
            }
        }).start();
    }
        ...
        api.uploadDataset(namaBody, part).enqueue(new Callback<DatasetItem>() {
            ...
            public void onResponse(Call<DatasetItem> call, retrofit2.Response<DatasetItem> resp) {
                if (!resp.isSuccessful() || resp.body() == null) {
                    ...
                    Toast.makeText(DatasetListttt.this,
            ...
            public void onFailure(Call<DatasetItem> call, Throwable t) {
                if (progress != null) progress.dismiss();
                Toast.makeText(DatasetListttt.this, "Error upload: " + t.getMessage(), Toast.LENGTH_LONG).show();
            }
        });
    }
    ...
    private void runNext(int datasetId, int kfold, List<String> queue,
        ...
        startTrainingSingle(
                datasetId, kfold, stream, summaryOnly, cmOnly,
    ...
    private void startTrainingSingle(int datasetId, int kfold, String stream,
                                     ...
                                     int streamIndex, int totalStreams,
                                     Runnable onDone) {
        // Selaraskan dengan worker.py
        int epochs = (kfold > 1) ? 5 : 10;
        String task = (kfold > 1) ? "kfold" : "split";

        // Pastikan flag sesuai mode terpilih
        boolean kfoldSummaryOnly = (kfold > 1) && summaryOnly; // true untuk k>1
        boolean cmOnlyFlag       = (kfold <= 1) && cmOnly;      // true untuk split

        StartTrainRequest body = new StartTrainRequest(kfold, task, epochs);
        body.stream = stream;  // "color" | "texture" | "fused"
        body.kfold_summary_only = kfoldSummaryOnly;
        body.cm_only = cmOnlyFlag;
                ...
                .enqueue(new Callback<TrainStatus>() {
                    @Override public void onResponse(Call<TrainStatus> call, retrofit2.Response<TrainStatus> resp) {
                        if (!resp.isSuccessful()) {
                            if (progress!=null) progress.dismiss();
                            Toast.makeText(DatasetListttt.this, "Gagal mulai ("+stream+"): "+resp.code(), Toast.LENGTH_LONG).show();
                            onDone.run();
                            return;
                        }
                        // Skala progress: total tahapan = totalStreams * folds
                        int folds = Math.max(1, body.kfold);
                        int totalStages = totalStreams * folds;
                        int baseStageIndex = streamIndex * folds;
                    ...
                    @Override public void onFailure(Call<TrainStatus> call, Throwable t) {
                        if (progress!=null) progress.dismiss();
                        Toast.makeText(DatasetListttt.this, "Error start ("+stream+"): "+t.getMessage(), Toast.LENGTH_LONG).show();
                        onDone.run();
                    }
                });
    }
...
//    }
            ...
            @Override public void run() {
                RetrofitClient.api().getTrainStatus(datasetId).enqueue(new Callback<TrainStatus>() {
                    @Override public void onResponse(Call<TrainStatus> call, retrofit2.Response<TrainStatus> resp) {
                        ...
                        // Ambil nomor fold dari message, contoh: "Fold 3/5 ..."
                        ...
                        // Hitung progress keseluruhan (stream × fold)
    ...
    // Ambil angka fold dari pesan worker, mis. "Fold 2/5 ..." atau "Fold 4 ..."
